...
| Codex CLI | Yes | cctop Codex plugin `hooks/hooks.json` → plugin `cctop-shim.sh` → `cctop-hook --harness codex` | `source: "codex"`; session-id-keyed `codex-<session_id>.json` because one host PID can emit multiple conversations | Uses session ID for display/dedup; subagent ownership from `~/.codex/state_5.sqlite` hides subagent-owned sessions |
| Codex Desktop | Yes | Same cctop Codex plugin hook path, hosted by the Codex Desktop app | `source: "codex"` plus trusted bundle ID `com.openai.codex`; session-id-keyed `codex-<session_id>.json`; titles read from `~/.codex/session_index.jsonl` | Desktop app liveness/recency applies unless `ended_at` is set; disconnected sessions stay dormant during retention; archived/subagent-owned threads from `~/.codex/state_5.sqlite` filter visibility; memory/title helper sessions auto-hide; deep links use `codex://threads/<uuid>` |
...
- **Codex CLI**: Uses Codex's lifecycle hooks system (feature flag `[features].hooks` in `~/.codex/config.toml`, default-true). cctop installs a local Codex plugin as `cctop@cctop` from the app-owned marketplace root `~/.cctop/codex-plugin-marketplace/`. The plugin bundles five hook entries in `plugins/cctop-codex/hooks/hooks.json` (SessionStart, UserPromptSubmit, PreToolUse, PostToolUse, Stop), and each hook invokes the plugin's `cctop-shim.sh`, which execs `cctop-hook <Event> --harness codex`. config.toml is only patched when there's something to fix: (a) strip any `codex_hooks` line, which triggers Codex's startup warning whenever loaded; (b) override an explicit `hooks = false` opt-out so the install actually fires. Legacy app-installed `~/.codex/hooks.json` cctop entries are removed during plugin install to avoid duplicate hook execution. `isInstalled()` treats an unset flag as installed (Codex default) — only an explicit opt-out under `[features]` counts as not installed.
...
- `plugins/cctop-codex/` - Codex plugin with diagnostics skill and bundled hook shim
...
**Codex CLI path:**
1. Codex fires lifecycle hook commands from the enabled cctop Codex plugin
2. Plugin `cctop-shim.sh` dispatches to `cctop-hook <Event> --harness codex`
3. `cctop-hook` writes session files to `~/.cctop/sessions/`

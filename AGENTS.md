...
- A desktop `terminal.bundle_id` is proof of desktop hosting only for `cc` -> Claude Desktop; nil-source legacy files keep bundle-first trust. Codex does not carry reliable Desktop-vs-CLI bundle evidence, so persisted Codex bundles do not select focus or host behavior. Any other pairing — including `cc` + `com.openai.codex` — is leaked launcher environment, not identity. `opencode` and `pi` never trust desktop bundle IDs.
...
| Codex CLI | Yes | `~/.codex/hooks.json` → `~/.codex/cctop-shim.sh` → `cctop-hook --harness codex` | `source: "codex"`; session-id-keyed `codex-<session_id>.json` because one host PID can emit multiple conversations | Uses the shared Codex policy across all hosts: process or recent hook activity means active, disconnection means dormant, and 14 days without activity means finished. Archive state hides a thread and can make its safe worktree available to Cleanup. Structured `threads.source` hides `SubAgent` and `Internal` helpers. |
| Codex Desktop | Yes | Same Codex hook shim path, hosted by the Codex Desktop app | `source: "codex"`; persisted `com.openai.codex` metadata is not reliable host evidence; session-id-keyed `codex-<session_id>.json`; titles read from `~/.codex/session_index.jsonl` | Uses the shared Codex lifecycle and visibility policy. Direct terminal/editor metadata wins for focus. Otherwise cctop opens the exact task in the installed Codex app. If that fails, cctop reports an error instead of opening Finder. Archive placement is checked against rollout files when available. Memory and title helpers auto-hide. |
...
- **Warp**: Opens the session deep link the hook captured from `WARP_FOCUS_URL` (Warp v0.2026.05.27+); Warp raises the window and focuses the exact pane. `WarpFocusLink` enforces the exact `<channel-scheme>://session/<32 hex>` shape with a channel-scheme allowlist at both capture and focus time (session JSON is user-writable), and derives the activation bundle ID from the scheme. All `dev.warp.*` channel bundle IDs resolve to the Warp host, not just stable. Activation runs before opening the link because Warp silently ignores stale UUIDs; sessions without a captured link (older Warp) fall back to app activation.
...
- If `terminal.bundle_id` is a desktop app that is not trusted for the harness (`opencode`/`pi` with any desktop bundle, `cc` with `com.openai.codex`, or `codex` with any persisted desktop bundle), keep the harness identity and debug process lifecycle. Child tools inherit GUI bundle IDs from the launching environment.

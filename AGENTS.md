...
| Codex Desktop | Yes | Same Codex hook shim path, hosted by the Codex Desktop app | `source: "codex"` plus trusted bundle ID `com.openai.codex`; session-id-keyed `codex-<session_id>.json`; titles read from `~/.codex/session_index.jsonl` | Uses the same lifecycle, archive, Cleanup, helper, badge, and notification policy as every Codex host. The bundle ID and app-server evidence are focus inputs only. Archive placement is checked against rollout files when available. Memory and title helpers auto-hide. Deep links use `codex://threads/<uuid>`. |

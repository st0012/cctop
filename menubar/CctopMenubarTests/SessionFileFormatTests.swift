...
final class SessionFileFormatTests: XCTestCase {
    ...
    func testDedupCurrentCodexFileBeatsLegacyNilSourcePidDuplicate() {
        let legacyPidKeyed = candidate(
            sessionId: "conv-a", pid: 12528, bundleId: nil, lifecycleRank: 0,
            source: nil,
            lastActivity: Date(timeIntervalSince1970: 3_000),
            mtime: Date(timeIntervalSince1970: 3_000),
            path: "/12528.json"
        )
        let codexKeyed = candidate(
            sessionId: "conv-a", pid: 12528, bundleId: nil, lifecycleRank: 0,
            source: Session.codexSource,
            lastActivity: Date(timeIntervalSince1970: 2_000),
            mtime: Date(timeIntervalSince1970: 2_000),
            path: "/codex-conv-a.json"
        )

        let result = deduped([legacyPidKeyed, codexKeyed])
        XCTAssertEqual(result.count, 1)
        XCTAssertEqual(result.first?.source, Session.codexSource)
    }

    @MainActor
    func testSessionManagerHidesLegacyNilSourcePidDuplicateWhenCodexFileExists() throws {
        let root = NSTemporaryDirectory() + "cctop-codex-legacy-pid-dedupe-\(UUID().uuidString)"
        let sessionsDir = (root as NSString).appendingPathComponent("sessions")
        let historyDir = (root as NSString).appendingPathComponent("history")
        try FileManager.default.createDirectory(atPath: sessionsDir, withIntermediateDirectories: true)
        try FileManager.default.createDirectory(atPath: historyDir, withIntermediateDirectories: true)
        ...
        var legacyPidKeyed = Session(
            sessionId: "conv-a", projectPath: "/tmp/p", branch: "main", terminal: TerminalInfo(program: "")
        )
        legacyPidKeyed.pid = pid
        legacyPidKeyed.lastActivity = Date(timeIntervalSince1970: 3_000)
        let legacyPath = (sessionsDir as NSString).appendingPathComponent("\(pid).json")
        try legacyPidKeyed.writeToFile(path: legacyPath)

        var codexKeyed = Session(
            sessionId: "conv-a", projectPath: "/tmp/p", branch: "main",
            terminal: TerminalInfo(bundleId: HostAppBundleID.codexDesktop)
        )
        codexKeyed.source = Session.codexSource
        codexKeyed.lastActivity = Date(timeIntervalSince1970: 2_000)
        codexKeyed.lastWrittenByHookVersion = Config.hookVersion
        let codexPath = (sessionsDir as NSString).appendingPathComponent("codex-conv-a.json")
        try codexKeyed.writeToFile(path: codexPath)
        ...
        XCTAssertEqual(manager?.sessions.first?.source, Session.codexSource)
        XCTAssertTrue(FileManager.default.fileExists(atPath: codexPath))

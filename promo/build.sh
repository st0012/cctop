...
set -euo pipefail
cd "$(dirname "$0")"
VIDEO="${1:-launch}"; PORT="${PORT:-8123}"; SCALE="${SCALE:-2}"; FPS=30; DUR="${DUR:-27.8}"; W="${W:-1920}"; H="${H:-1080}"
...
node engine/render.mjs --url="http://127.0.0.1:$PORT/videos/$VIDEO.html" \
  --out="$OUT/frames" --duration="$DUR" --fps="$FPS" --scale="$SCALE" --width="$W" --height="$H"
OW="$W" OH="$H" bash engine/encode.sh "$OUT/frames" "$OUT/$VIDEO.mp4"
